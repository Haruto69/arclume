      ...
      <AppShell>
        <div className="space-y-6">
          <div>
            ...
            <p className="mt-1 text-muted-foreground">Search externally ingested roles. Jobs provided by Remotive API.</p>
          </div>

          <section className="grid gap-3 rounded-lg border border-border bg-card p-4 md:grid-cols-2 lg:grid-cols-5">
          ...
          {loading ? <SkeletonList /> : error && jobs.length === 0 ? (
            <div className="rounded-lg border border-dashed border-danger-border bg-danger-muted p-8 text-center">
              ...
              <p className="mt-2 text-danger">Check the backend connection or adjust a filter to try again.</p>
            </div>
          ) : jobs.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border-strong bg-card p-8 text-center"><h2 className="text-xl font-semibold">No jobs found</h2><p className="mt-2 text-muted-foreground">Try relaxing your search filters.</p></div>
          ) : (
...
function Input({ label: inputLabel, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <label className="text-sm font-medium text-secondary-foreground">{inputLabel}<input type="search" value={value} onChange={(event) => onChange(event.target.value)} className="mt-1 w-full rounded-md border border-input-border bg-input px-3 py-2" /></label>;
}
...
function Select({ label: selectLabel, value, values, onChange }: { label: string; value: string; values: string[]; onChange: (value: string) => void }) {
  return <label className="text-sm font-medium text-secondary-foreground">{selectLabel}<select value={value} onChange={(event) => onChange(event.target.value)} className="mt-1 w-full rounded-md border border-input-border bg-input px-3 py-2"><option value="">Any</option>{values.map((item) => <option key={item} value={item}>{labelize(item)}</option>)}</select></label>;
}
...
function JobCard({ job, application, checking, trackingBusy, trackingThis, onCheckFit, onTrack }: {
  ...
  return (
    <article className="flex min-h-64 min-w-0 flex-col rounded-lg border border-border bg-card p-5">
      <div className="min-w-0 flex-1 space-y-3">
        <div className="flex min-w-0 flex-wrap items-start justify-between gap-2">
          <div className="min-w-0"><h2 className="break-words text-xl font-semibold text-foreground">{job.title}</h2><p className="break-words text-sm text-muted-foreground">{job.company}</p></div>
          {application && <span className="shrink-0 rounded-full bg-muted px-2.5 py-1 text-xs font-semibold text-foreground">{label(application.status)}</span>}
        </div>
        <p className="break-words text-sm text-secondary-foreground">{job.location || "Location unavailable"} / {label(job.workMode)} / {label(job.employmentType)}</p>
        {job.salaryRange && <p className="break-words text-sm text-success">{job.salaryRange}</p>}
        <p className="line-clamp-4 break-words text-sm leading-6 text-muted-foreground">{stripHtml(job.description || job.requirements || "No description available.")}</p>
      </div>
      <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
        <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
          <span className="break-words text-subtle-foreground">Source: {job.sourceProvider || "Remotive"}</span>
          {job.externalUrl && <a href={job.externalUrl} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-10 items-center font-medium text-link underline decoration-border-strong underline-offset-4 hover:decoration-foreground">Original job</a>}
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" disabled={checking} onClick={onCheckFit} className="min-h-10 rounded-md bg-primary px-3 py-2 font-semibold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50">{checking ? "Checking..." : "Check my fit"}</button>
          {!application && (
            <>
              <button type="button" disabled={trackingBusy} onClick={() => onTrack("SAVED")} className="min-h-10 rounded-md border border-border-strong px-3 py-2 font-medium disabled:opacity-50">{trackingThis ? "Saving..." : "Save job"}</button>
              <button type="button" disabled={trackingBusy} onClick={() => onTrack("APPLIED")} className="min-h-10 rounded-md border border-success-border px-3 py-2 font-medium text-success disabled:opacity-50">{trackingThis ? "Updating..." : "Mark applied"}</button>
            </>
          )}
          {application?.status === "SAVED" && <button type="button" disabled={trackingBusy} onClick={() => onTrack("APPLIED")} className="min-h-10 rounded-md border border-success-border px-3 py-2 font-medium text-success disabled:opacity-50">{trackingThis ? "Updating..." : "Mark applied"}</button>}
          {application && <Link href="/applications" className="inline-flex min-h-10 items-center rounded-md border border-border-strong px-3 py-2 font-medium hover:border-foreground">View tracking</Link>}
        </div>
      </div>
    </article>
  );
    ...
    <Dialog labelledBy="job-fit-title" onClose={onClose} panelClassName="max-w-2xl">
      <div className="mb-4 flex min-w-0 items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="break-words text-sm text-muted-foreground">{state.job.title} at {state.job.company}</p>
        </div>
      ...
      {state.loading ? (
        <div className="rounded-lg border border-border bg-background p-6 text-secondary-foreground">Calculating your fit...</div>
      ) : state.error ? (
    ...
    <div className="space-y-5">
      <div className="rounded-lg border border-border bg-background p-5">
        <div className="flex min-w-0 flex-wrap items-center gap-3">
          ...
          <span className="rounded-full bg-primary px-3 py-1 text-sm font-bold text-primary-foreground">{score}% match</span>
        </div>
        <p className="mt-1 break-words text-sm text-muted-foreground">{result.companyName}</p>
        <div className="mt-4 h-2 overflow-hidden rounded-full bg-muted" role="progressbar" aria-label="Job match score" aria-valuemin={0} aria-valuemax={100} aria-valuenow={score}>
          <div className="h-full bg-primary" style={{ width: score + "%" }} />
        </div>
        <p className="mt-4 break-words text-sm leading-6 text-secondary-foreground">{result.explanation}</p>
      </div>
...
function SkillGroup({ title, skills, tone }: { title: string; skills: string[]; tone: "good" | "warn" }) {
  const skillClass = tone === "good" ? "bg-success-muted text-success" : "bg-warning-muted text-warning";
  return (
    <div className="min-w-0 rounded-lg border border-border bg-background p-4">
      <h4 className="text-sm font-semibold text-foreground">{title}</h4>
      <div className="mt-3 flex min-w-0 flex-wrap gap-2">
        {skills.length === 0 ? <span className="text-sm text-subtle-foreground">None found</span> : skills.map((skill, index) => <span key={skill + index} className={["max-w-full break-words rounded-full px-2.5 py-1 text-xs font-medium", skillClass].join(" ")}>{skill}</span>)}
      </div>
    </div>
  );
}
